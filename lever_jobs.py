...
def lever_jobs():
    ...
    ryan_keywords = ["cybersecurity", "siem", "splunk", "threat", "vulnerability", "information security"]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for job in data:
                    ...
                    #     list_content += f" {item.get('text', '')} {item.get('content', '')}"
                    
                    #check content for keywords
                    raw_content = f"{desc_plain} {add_plain}"
                    ...
                    #deal with locations
                    onsite_name = cats.get('location') or ""
                    ...
                    country_code = (job.get('country') or "").lower()
                    
                    
                    all_loc_strings = list(set([onsite_req, country_code] + [str(loc).lower().strip() for loc in all_loc_list if loc]))
                    ...
                    ryan_match = ryan_key_match and (ryan_loc_check or (is_remote and any(t in all_loc_strings for t in ryan_loc)))
                    mik_match = mik_key_match and (mik_loc_check or (is_remote and any(t in all_loc_strings for t in mik_loc)))                
